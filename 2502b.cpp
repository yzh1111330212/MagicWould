...
void bag2(){//背包检查工具（是否存在错误，如：空，114514个） 
	for(int i=0;i<50;i++){
		if(bag[i][1]==0){
			bag[i][0]=0;
		}
...
void study(){
	cout<<"[1]一级魔法书  [E]退出"<<endl;
	cin>>tempchar;
	if(tempchar=='1'){
        if(!bagfind(4)){
            MessageBox(0,"错误:\nBOOKO_NOT_FIND\n未找到一级魔法书！","Error",MB_OK+16);
			return;
        }
		cout<<"正在学习...\n";
		for(int i=0;i<100;i++){
...
void game(){
	...
	while(x!='X'&&x!='x'){
		system("cls");
		slowly("[O]生成代码  [S]商店  [E]背包  [F]决斗  [X]退出 [A]学习  金币:");
		cout<<jb;
		slowly("    血量:");
		cout<<xl<<endl;
